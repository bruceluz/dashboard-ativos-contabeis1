...
def padronizar_nome_filial(nome_filial):
    if not isinstance(nome_filial, str): return "Não Identificado"
...
def corrigir_filiais_nao_identificadas(df_arquivo):
    if df_arquivo.empty: return df_arquivo
...
with st.sidebar:
    ...
    email1 = "pessoa1@suaempresa.com"
    email2 = "pessoa2@suaempresa.com"
...
if uploaded_files:
    ...
    for i, file in enumerate(uploaded_files):
        ...
        if dados is not None and not dados.empty: all_data.append(dados)
        if erro: errors.append(erro)
    ...
    if all_data:
        ...
        if not dados_filtrados.empty and eixo_x:
            opcoes_foco = ["Mostrar Todos"] + sorted(dados_filtrados[eixo_x].unique().tolist())
            foco_selecionado = st.selectbox(f"Focar em um(a) {eixo_x} específico(a) (opcional):", opcoes_foco)
        ...
        if not dados_filtrados.empty and eixo_x and eixos_y:
            ...
            if foco_selecionado != "Mostrar Todos":
                dados_para_grafico = dados_para_grafico[dados_para_grafico[eixo_x] == foco_selecionado]
            ...
            fig = px.bar(
                ...
                labels={eixo_x: eixo_x, 'Valor': "Soma dos Valores", 'Métrica': "Métrica Financeira"},
        ...
        st.download_button(label="📥 Baixar Relatório Filtrado (Excel)", data=output.getvalue(), file_name="relatorio_ativos_filtrado.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
