...
class PDF(FPDF):
    def header(self):
        ...
        if os.path.exists(font_path) and font_name not in self.font_families:
            try:
                ...
                st.warning(f"Não foi possível carregar a fonte '{font_name}'. Usando '{fallback_font}'.")
                self.set_font(fallback_font, "B", 12)
        else:
             self.set_font(font_name if font_name in self.font_families else fallback_font, "B", 12)
        ...
        self.set_font(font_name if font_name in self.font_families else fallback_font, "B", 20)
    ...
    def footer(self):
        ...
        font_name = "LiberationSans"
        fallback_font = "Arial"
        self.set_font(font_name if font_name in self.font_families else fallback_font, "I", 8)
...
def limpar_valor(valor_str):
    """Converte uma string de valor (ex: '1.119.850,39' ou '-') para float."""
    try:
        # Remove caracteres não numéricos, exceto vírgula e sinal de menos
...
def processar_linhas_do_arquivo(linhas_texto, nome_arquivo):
    ...
    for linha in linhas_texto:
        ...
        if len(colunas) >= 2 and colunas[0].startswith("1.2.3."):
            ...
            if dados_item_atual:
                try:
                    dados_item_atual["Valor original"] = limpar_valor(colunas[2])
                    dados_item_atual["Valor atualizado"] = limpar_valor(colunas[3])
                    dados_item_atual["Deprec. do mês"] = limpar_valor(colunas[4])
                    dados_item_atual["Deprec. Acumulada"] = limpar_valor(colunas[6])
                    dados_item_atual["Valor Residual"] = dados_item_atual["Valor atualizado"] - dados_item_atual["Deprec. Acumulada"]
            ...
            try:
                ...
                data_aquisicao = pd.to_datetime(colunas[7], format='%d/%m/%Y', errors='coerce')
...
def processar_planilha(file_content, file_name):
    ...
    try:
        ...
        dados_processados = processar_linhas_do_arquivo(linhas_texto, file_name)
        ...
        if dados_processados:
            ...
            df_final = df_final.reindex(columns=colunas_ordenadas, fill_value="")
...
def criar_pdf_completo(buffer, df_filtrado, fig_plotly):
    ...
    font_name = "LiberationSans" if "LiberationSans" in pdf.font_families else "Arial"

    if fig_plotly:
        try:
            pdf.set_font(font_name, "B", 14)
            pdf.cell(0, 10, "Gráfico Analítico", 0, 1, 'L')
            img_bytes = fig_plotly.to_image(format="png", width=1000, height=500, scale=2)
            img_buffer = BytesIO(img_bytes)
            pdf.image(img_buffer, x=10, y=pdf.get_y(), w=277)
            pdf.ln(135)
        except Exception as e:
            pdf.set_font(font_name, "", 10)
            pdf.multi_cell(0, 10, f"Não foi possível renderizar o gráfico no PDF: {e}", 0, 'L')
    ...
    col_widths = {'Filial': 60, 'Descrição da conta': 100, 'Valor atualizado': 38, 'Deprec. Acumulada': 40, 'Valor Residual': 39}
    ...
    for _, row in df_agregado.iterrows():
        ...
        pdf.cell(col_widths['Descrição da conta'], 10, str(row['Descrição da conta']), 1, 0, 'L')
        pdf.cell(col_widths['Valor atualizado'], 10, formatar_valor(row['Valor atualizado']), 1, 0, 'R')
        pdf.cell(col_widths['Deprec. Acumulada'], 10, formatar_valor(row['Deprec. Acumulada']), 1, 0, 'R')
        pdf.cell(col_widths['Valor Residual'], 10, formatar_valor(row['Valor Residual']), 1, 0, 'R')
        pdf.ln()
...
uploaded_files = st.file_uploader("Escolha os arquivos de relatório de ativos", type=['xlsx', 'xls', 'txt'], accept_multiple_files=True)
...
if uploaded_files:
    ...
    if all_data:
        ...
        st.success(f"Processamento finalizado! {len(all_data)} arquivo(s) com dados detalhados carregados, totalizando {len(dados_combinados):,} registros.")
        ...
        with col1:
            selecao_arquivo = st.multiselect("Filtrar por Arquivo:", arquivos_options, default=arquivos_options)
        with col2:
            selecao_filial = st.multiselect("Filtrar por Filial:", filiais_options, default=filiais_options)
        with col3:
            selecao_categoria = st.multiselect("Filtrar por Descrição da Conta:", categorias_options, default=categorias_options)
        ...
        with tab1:
            ...
            colunas_formatar = ['Valor original', 'Valor atualizado', 'Deprec. do mês', 'Deprec. Acumulada', 'Valor Residual']
        ...
        with tab2:
            ...
            st.dataframe(analise_filial, use_container_width=True, column_config={"Valor_Total": st.column_config.NumberColumn(format="R$ %.2f")})
        with tab3:
            ...
            st.dataframe(analise_categoria, use_container_width=True, column_config={"Valor_Total": st.column_config.NumberColumn(format="R$ %.2f")})
        ...
        with col_graf2:
            eixo_x = st.selectbox("Agrupar por (Eixo X):", ["Filial", "Descrição da conta", "Arquivo"])
        with col_graf3:
            if tipo_grafico == "Pizza":
                eixos_y = [st.selectbox("Analisar Valor:", opcoes_eixo_y, index=0)]
            else:
        ...
        if not dados_filtrados.empty and eixo_x and eixos_y:
            ...
            if tipo_grafico == "Barras":
                fig_plotly = px.bar(dados_agrupados, x=eixo_x, y=eixos_y, text_auto='.2s', barmode='group', title=f'Análise de {", ".join(eixos_y)} por {eixo_x}')
            elif tipo_grafico == "Linhas":
                fig_plotly = px.line(dados_agrupados, x=eixo_x, y=eixos_y, markers=True, title=f'Análise de {", ".join(eixos_y)} por {eixo_x}')
            elif tipo_grafico == "Pizza":
                fig_plotly = px.pie(dados_agrupados, names=eixo_x, values=eixos_y[0], hole=0.3, title=f'Distribuição de {eixos_y[0]} por {eixo_x}')
                fig_plotly.update_traces(textposition='outside', textinfo='percent+label')

            if fig_plotly:
                fig_plotly.update_layout(uniformtext_minsize=8, uniformtext_mode='hide', legend_title_text='')
                st.plotly_chart(fig_plotly, use_container_width=True)
                st.session_state.figura_plotly = fig_plotly
            else:
                st.session_state.figura_plotly = None
        else:
            st.info("Não há dados para exibir no gráfico com os filtros selecionados.")
            st.session_state.figura_plotly = None
        ...
        with col_download2:
            if st.session_state.figura_plotly:
                output_pdf = BytesIO()
                criar_pdf_completo(output_pdf, dados_filtrados, st.session_state.figura_plotly)
                st.download_button(
                ...
                st.warning("Gere um gráfico primeiro para poder exportar o relatório em PDF.")

    if errors:
        st.warning("Avisos durante o processamento:")
        for error_msg in errors:
            if "Nenhum item de ativo detalhado foi encontrado" in error_msg:
                st.info(error_msg) # Mostra como informação, não como erro
            else:
                st.error(error_msg) # Mostra erros críticos
    ...
    st.info("Por favor, carregue um ou mais arquivos de relatório para iniciar a análise.")
