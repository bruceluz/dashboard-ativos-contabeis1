...
### ALTERAÇÃO: Função do PDF agora agrega os dados ###
def criar_pdf_relatorio(buffer, df_filtrado, grafico_fig):
    ...
    # Define as larguras das colunas para a tabela no PDF
...
if uploaded_files:
    ...
    if all_data:
        ...
        with col_download1:
            ...
            for col in ['Valor Original', 'Valor Atualizado', 'Deprec. no mês', 'Deprec. no Exercício', 'Deprec. Acumulada', 'Valor Residual']:
                df_display_excel[col] = df_display_excel[col].apply(formatar_valor)
            with pd.ExcelWriter(output_excel, engine='xlsxwriter') as writer:
                df_display_excel.to_excel(writer, sheet_name='Dados_Filtrados', index=False)
            st.download_button(
...
st.markdown("---")
st.caption("Desenvolvido para General Water | v22.0 - Suporte via Teams")
