...
def processar_linhas_detalhadas(linhas_texto, nome_arquivo):
    ...
    for linha in linhas_texto:
        ...
        # Identifica linha de filial
            ...
            filial_atual = linha_strip.replace('Filial :', '').split('-')[0].strip()
        ...
        # Identifica linha de dados detalhados (começa com código numérico de filial)
            ...
            try:
                ...
                if len(partes) >= 8 and '/' in partes[7] and len(partes[7].split('/')) == 3:
                    data_aquisicao = pd.to_datetime(partes[7], format='%d/%m/%Y', errors='coerce')
        ...
        # Identifica linha de valores em R$ (vinculada ao item anterior)
            ...
            try:
                if len(partes) >= 8:
                    dados_item_atual["Valor original"] = limpar_valor(partes[2])
                    dados_item_atual["Valor atualizado"] = limpar_valor(partes[3])
                    dados_item_atual["Deprec. do mês"] = limpar_valor(partes[4])
                    dados_item_atual["Deprec. do exercício"] = limpar_valor(partes[5])
                    dados_item_atual["Deprec. Acumulada"] = limpar_valor(partes[6])
...
def processar_planilha(file_content, file_name):
    ...
    try:
        ...
        dados_processados = processar_linhas_detalhadas(linhas_texto, file_name)
...
if uploaded_files:
    ...
    if all_data:
        ...
        st.success(f"Processamento finalizado! {len(all_data)} arquivo(s) carregados, totalizando {len(dados_combinados):,} registros.")
